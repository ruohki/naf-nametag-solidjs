...
*/
      ...
      Module.Pd.receiveList = function (source, list) {
       ...
       if (source == "r_boid") {
          if (list[0] == 0) {
            var pdxrch1 = document.getElementById("pdxr-ch1");

            pdxrch1.setAttribute(
              "resonance-audio-src",
              "position",
              "" +
                parseFloat(list[1]).toFixed(2) * 10 +
                " " +
                parseFloat(list[2]).toFixed(2) * 10 +
                " " +
                parseFloat(list[3]).toFixed(2) * 10+
                ""
            );
          }
          if (list[0] == 1) {
            var pdxrch2 = document.getElementById("pdxr-ch2");
            pdxrch2.setAttribute(
              "resonance-audio-src",
              "position",
              "" +
                parseFloat(list[1]).toFixed(2) * 10 +
         ...
         /*
         if (list[0] == 2) {
            var pdxrch3 = document.getElementById("pdxr-ch3");
            pdxrch3.setAttribute(
              "resonance-audio-src",
              "position",
              "" +
                parseFloat(list[1]).toFixed(2) * 10 +
        ...
        if (list[0] == 3) {
            var pdxrch4 = document.getElementById("pdxr-ch4");
            pdxrch4.setAttribute(
              "resonance-audio-src",
              "position",
              "" +
                parseFloat(list[1]).toFixed(2) * 10 +
  ...
  for (let line of lines) {
    ...
    switch (type) {
          ...
          switch (args[4]) {
            ...
            case "dac~":
              ...
              pdxrch1.setAttribute(
                ...
                "position",
                "" +
                  (thisEl.components["position"].attrValue.x - 1) +
                  " " +
                  thisEl.components["position"].attrValue.y +
                  " " +
                  thisEl.components["position"].attrValue.z +
                  ""
              );
      ...
      case "#X text":
        ...
        break;
      case "#X connect":
        const elObj1 = thisEl.querySelector("#PdObj" + args[2]);
        const elObj2 = thisEl.querySelector("#PdObj" + args[4]);

        let LineStart = elObj1.components["position"].attrValue;
        let LineEnd = elObj2.components["position"].attrValue;
...
AFRAME.registerComponent("pdxr", {
  ...
  update: function () {
    ...
    var ch1 = document.createElement("a-pdxr-audiochannel");
    ch1.setAttribute("id", "pdxr-ch1");
    ch1.setAttribute("resonance-audio-src", "room", resonanceRoom);
    ch1.setAttribute("resonance-audio-src", "gain", 1.);
  //  ch1.setAttribute("resonance-audio-src", "visualize", this.data.visualize);
    ch1.setAttribute("resonance-audio-src", "visualize", true);
    ch1.setAttribute("resonance-audio-src", "minDistance", 0.5);
    ch1.setAttribute("resonance-audio-src", "maxDistance", 100);


    el.appendChild(ch1);

    if (this.data.stereo) {
      var ch2 = document.createElement("a-pdxr-audiochannel");
      ch2.setAttribute("id", "pdxr-ch2");
      ch2.setAttribute("resonance-audio-src", "room", resonanceRoom);
      ch2.setAttribute("resonance-audio-src", "gain", 1.);
     // ch2.setAttribute("resonance-audio-src", "visualize", this.data.visualize);
      ch2.setAttribute("resonance-audio-src", "visualize", true);
      ch2.setAttribute("resonance-audio-src", "minDistance", 0.5);
      ch2.setAttribute("resonance-audio-src", "maxDistance", 100);
...
AFRAME.registerPrimitive("a-pdxr-audiochannel", {
  defaultComponents: {
    "resonance-audio-src": {
      loop: "true",
      autoplay: "true",
